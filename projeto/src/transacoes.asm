# ============================================================
# transacoes.asm � transa��es detalhadas + formatador simples
# layout de 32 bytes por transa��o:
#  [0]       : 1 = ativo, 0 = vazio
#  [1..8]    : conta destino (string, at� 8, termina em 0)
...
mostrar_transacoes_debito:
    ...
    addu  $s2, $s2, $t1          # s2 = in�cio do bloco do cliente
...
# vers�o alinhada (n�o bagun�a o $sp byte a byte)
# ------------------------------------------------------------
formatar_centavos:
    addiu $sp, $sp, -32
    sw    $ra, 28($sp)
    ...
    li    $t0, ' '
    sb    $t0, 2($s0)
    addiu $s1, $s0, 3                  # s1 = onde vamos escrever n�mero
    ...
    # usar parte final do buffer pra guardar d�gitos ao contr�rio
    addiu $s3, $s0, 24
    move  $t2, $s3
...
fc_copy_back:
    # t2 aponta 1 al�m do �ltimo d�gito
    addiu $t2, $t2, -1
fc_copy_loop:
    lb    $t4, 0($t2)
    sb    $t4, 0($s1)
